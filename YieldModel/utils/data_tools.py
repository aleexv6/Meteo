...
def drop_unique(data):
    ...
    """
    return data[data.duplicated(subset=['DEP'], keep=False)]
...
def remove_county_fixed_effect(train_data, test_data):
    ...
    Returns:
        pandas.DataFrame: the training data
        pandas.Series: the DEP county codes for the training data
        pandas.DataFrame: the testing data
        pandas.Series: the DEP county codes for the testing data
    ...
    train_data = train_data[train_data['DEP'].isin(counties)].copy()
    test_data = test_data[test_data['DEP'].isin(counties)].copy()

    train_dep = train_data['DEP']
    test_dep = test_data['DEP']

    train_data['yield'] -= county_fixed_effect.loc[train_dep, 'mean'].values
    train_data['yield'] /= county_fixed_effect.loc[train_dep, 'std'].values

    test_data['yield'] -= county_fixed_effect.loc[test_dep, 'mean'].values
    test_data['yield'] /= county_fixed_effect.loc[test_dep, 'std'].values

    return train_data, train_dep, test_data, test_dep, county_fixed_effect
...
def reapply_county_fixed_effect(labels, predictions, dep,
    ...
    Parameters:
        ...
        dep (pandas.Series): the DEP county codes
    ...
    labels += county_fixed_effect.loc[dep, 'mean'].values
